        ...
        for i in range(self.args.trojan_outter_epochs):
            ...
            for j in range(self.args.trojan_inner_epochs):
                ...
                for data in tobe_poison_sup_dataset:
                    ...
                    trojan_trigger_data = Data(x=poison_x, edge_index=trojan_edges, edge_weight = trojan_weights)
                    trojan_trigger_data_list.append(trojan_trigger_data)
                ...
                for batch_id, data in enumerate(trojan_trigger_data_list):
                    data = data.to(self.device)
                    trojan_edges = data.edge_index
                    trojan_weights = data.edge_weight
                    poison_feats = data.x
                    loss_homo = self.homo_loss(trojan_edges[:,:int(trojan_edges.shape[1]/2)],\
                                        trojan_weights,\
                                        poison_feats,\
                                        self.homo_boost_thrd)      

                    poison_data = trojan_sup_dataset[batch_id].to(self.device)
                    poison_data = Batch.from_data_list([poison_data])
                    graph_emb = self.pretrain_gnn(poison_data.x, poison_data.edge_index, poison_data.batch, prompt = self.prompt_generator, prompt_type = 'Gprompt')
                    # dt_predict = self.downstream_tasker(graph_emb)
                    center, class_counts = center_embedding(graph_emb, poison_data.y, self.num_classes)
                    ...
                    loss_sup = criterion(graph_emb, center.detach(), poison_data.y)  
                    
                    loss = loss_sup+loss_homo
                    
                    optimizer_trigger.zero_grad()
                    loss.backward()
                    optimizer_trigger.step()

                    loss_trojan_batch += loss
                if((i) % printN == 0):
                    print("{}/{} Loss Trojan: {:.4f}".format(j, self.args.trojan_inner_epochs,loss_trojan_batch))



                benign_sup_dataloader = DataLoader(self.support_dataset.to_data_list(), batch_size=10, shuffle=False)
                loss_sup_batch = 0.

                accumulated_centers = None
                accumulated_counts = None

                for batch_id, data in enumerate(benign_sup_dataloader):
                    data = data.to(self.device)
        ...
        for i in range(self.args.trojan_outter_epochs):
            ...
            for j in range(self.args.trojan_inner_epochs):
                ...
                for batch_id, data in enumerate(trojan_trigger_data_list): 
                    ...
                    poison_data = trojan_sup_dataset[batch_id].to(self.device)
                    poison_data = Batch.from_data_list([poison_data])
                    prompted_graph = self.prompt_generator(poison_data)
                    graph_emb = self.pretrain_gnn(prompted_graph.x, prompted_graph.edge_index, prompted_graph.batch)
                    dt_predict = self.downstream_tasker(graph_emb)
                    loss_sup = F.cross_entropy(dt_predict,data.y)

                    loss = loss_homo + loss_sup
