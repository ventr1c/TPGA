    ...
    def transfer_to_trojan_test_dataset(self, dataset,idx_atk, prompt_type, device = None):
        if(device != None):
            ...
            for data in tobe_poison_dataset:
                ...
                poison_x, poison_edge_index, _ = self.inject_trigger_rand(idx_attach, data.x, data.edge_index)
                poison_x, poison_edge_index = poison_x.to(device), poison_edge_index.to(device)
                poison_y = torch.LongTensor([self.args.target_class]).to(device)
                poison_data = Data(x=poison_x, edge_index=poison_edge_index, y=poison_y).to(device)
