...
from prompt_graph.prompt import GPF, GPF_plus, LightPrompt,HeavyPrompt, Gprompt, GPPTPrompt, DiffPoolPrompt, SAGPoolPrompt, FrontAndHead
...
class BaseTask:
    def __init__(self, args, pre_train_model_path=None, gnn_type='TransformerConv', hid_dim = 128, num_layer = 2, dataset_name='Cora', prompt_type='GPF', epochs=100, shot_num=10, device : int = 1):
        self.args = args
    ...
    def initialize_optimizer(self):
        if self.prompt_type == 'None':
            ...
            self.pg_opi = optim.Adam(filter(lambda p: p.requires_grad, self.prompt.parameters()), lr=self.args.prompt_lr, weight_decay=self.args.prompt_weight_decay)
            self.answer_opi = optim.Adam(filter(lambda p: p.requires_grad, self.answering.parameters()), lr=self.args.answering_lr, weight_decay=self.args.answering_weight_decay)
        elif self.prompt_type in ['GPF', 'GPF-plus']:
            # model_param_group = []
            # model_param_group.append({"params": self.prompt.parameters()})
            # model_param_group.append({"params": self.answering.parameters()})
            # self.optimizer = optim.Adam(model_param_group, lr=0.0005, weight_decay=5e-4)
            self.pg_opi = optim.Adam(filter(lambda p: p.requires_grad, self.prompt.parameters()), lr=self.args.prompt_lr, weight_decay=self.args.prompt_weight_decay)
            self.answer_opi = optim.Adam(filter(lambda p: p.requires_grad, self.answering.parameters()), lr=self.args.answering_lr, weight_decay=self.args.answering_weight_decay)

        elif self.prompt_type in ['Gprompt', 'GPPT']:
            self.pg_opi = optim.Adam(self.prompt.parameters(), lr=self.args.prompt_lr, weight_decay=self.args.prompt_weight_decay)
    ...
    def initialize_prompt(self):
        if self.prompt_type == 'None':
            ...
            self.prompt = HeavyPrompt(token_dim=self.input_dim, token_num=1, cross_prune=0.1, inner_prune=0.01).to(self.device)
    ...
    def initialize_gnn(self):
        print(self.input_dim,self.hid_dim,self.output_dim)
        if self.gnn_type == 'GAT':
            self.gnn = GAT(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer, pool = self.args.pooling_type)
        elif self.gnn_type == 'GCN':
            self.gnn = GCN(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer, pool = self.args.pooling_type)
        elif self.gnn_type == 'GraphSAGE':
            self.gnn = GraphSAGE(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer, pool = self.args.pooling_type)
        elif self.gnn_type == 'GIN':
            self.gnn = GIN(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer, pool = self.args.pooling_type)
        elif self.gnn_type == 'GCov':
            self.gnn = GCov(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer, pool = self.args.pooling_type)
        elif self.gnn_type == 'GraphTransformer':
            self.gnn = GraphTransformer(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer, pool = self.args.pooling_type)
        else:
