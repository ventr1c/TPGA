...
def obtain_attach_nodes(seed,node_idxs, size):
...
def get_split_trojan_evaluation(num_samples: int, clean_test_ratio: float = 0.5, attack_ratio: float = 0.5, seed: int = 42, device=None):
    """Return indices for clean test and attack samples."""
    assert clean_test_ratio + attack_ratio == 1
    rs = np.random.RandomState(seed)
    perm = rs.permutation(num_samples)
    indices = torch.tensor(perm).to(device)
    clean_test_size = int(num_samples * clean_test_ratio)
    attack_size = num_samples - clean_test_ratio
    # indices = torch.randperm(num_samples)

    
    return {
        'clean_test': indices[:clean_test_size],
        'attack': indices[clean_test_size:]
    }
    ...
    if updates is not None:
        ...
        for p, g in zip(params, updates):
            ...
            if p is not None:
                ...
                # If gradient is None, no update is applied
                p.update = torch.zeros_like(p)
