...
class Backdoor:
    ...
    def trojan_ASR_test_evaluation(self):
        ...
        return test_acc, bkd_asr        
    
    def meta_backdoor(self, poison_x, poison_edge_index, idx_trojan, eps, step, random):
        """
        ...
        for _ in range(step):
            ...
            out = clone(node_embedding, poison_edge_index)
            
            # Compute the backdoor loss for the trojaned inputs
            backdoor_loss = self.criterion(out[idx_trojan], self.labels[idx_trojan])
    ...
    def GPPT_fit(self, PG, gnn, data, idx_train, idx_trojan, idx_unlabeled):
        ...
        for i in range(self.args.trojan_outter_epochs):
            ...
            for j in range(self.args.trojan_inner_epochs):
                ...
                update_poison_edge_weights = poison_edge_weights[poison_edge_weights>0.0]
                self.poison_x, self.poison_edge_index, self.poison_edge_weights = poison_x, updated_poison_edge_index, update_poison_edge_weights
                node_embedding = self.pretrain_gnn(poison_x, poison_edge_index, edge_weight = poison_edge_weights)
                ...
                loss_meta = self.meta_backdoor(poison_x, updated_poison_edge_index, idx_trojan, self.args.eps, self.args.step, self.args.random)

                loss = loss_homo + loss_inner + loss_meta
                ...
                test_acc, bkd_asr = self.trojan_ASR_test_evaluation()
            if((i) % printN == 0):
                print("{}/{} Loss inner: {:.4f} Loss inner bkd: {:.4f} Loss inner clean: {:.4f} Loss homo: {:.4f} Loss meta: {:.4f} ".format(i, self.args.trojan_outter_epochs,loss_inner, loss_inner_bkd, loss_inner_clean, loss_homo, loss_meta))
            if(test_acc > self.clean_acc):
                break
            ...
            # num_outter_choose = min(128, int(len(idx_unlabeled) * 0.5))
            # idx_outter = torch.cat([idx_trojan,idx_unlabeled[rs.choice(len(idx_unlabeled),size=num_outter_choose,replace=False)]])

            # trojan_feat, trojan_weights = self.trojan(data.x[idx_outter],self.args.thrd) # may revise the process of generate
            ...
            # trojan_edge = self.get_trojan_edge(len(data.x),idx_outter,self.args.trigger_size).to(self.device)

            # update_edge_weights = torch.cat([edge_weight,trojan_weights,trojan_weights])
            # update_feat = torch.cat([data.x,trojan_feat])
            # update_edge_index = torch.cat([data.edge_index,trojan_edge],dim=1)
            
            # update_edge_index = update_edge_index[:,update_edge_weights>0.0]
            # update_edge_weights = update_edge_weights[update_edge_weights>0.0]

            # node_embedding = self.pretrain_gnn(update_feat, update_edge_index)
            # out = self.prompt_generator(node_embedding, update_edge_index)
            
            # labels_outter = data.y.clone()
            # labels_outter[idx_outter] = self.args.target_class
            # loss_target = self.criterion(out[torch.concat((idx_train,idx_outter))], self.labels[torch.concat((idx_train,idx_outter))])
            # # loss_target = loss_target + 0.001 * constraint(self.device, self.prompt_generator.get_TaskToken())

            # loss_outter = loss_target 

            # if((i) % printN == 0):
            #     print("{}/{} \t\tLoss outter: {:.4f}".format(i, self.args.trojan_inner_epochs,loss_outter))
    ...
    def GPPT_fit_freeze_DT(self, PG, gnn, data, idx_train, idx_trojan, idx_unlabeled):
        ...
        for i in range(self.args.trojan_outter_epochs):
            ...
            if((i) % printN == 0):
                print("{}/{} Loss inner: {:.4f} Loss inner bkd: {:.4f} Loss inner clean: {:.4f} Loss homo: {:.4f}".format(i, self.args.trojan_outter_epochs,loss_inner, loss_inner_bkd, loss_inner_clean, loss_homo))
