...
class GPPTPrompt(torch.nn.Module):
    ...
    def __deepcopy__(self, memo):
        # Create a new instance of GPPTPrompt with the same parameters
        new_instance = GPPTPrompt(self.n_hidden, self.center_num, self.n_classes, self.device)

        # Copy the state of StructureToken
        new_instance.StructureToken.load_state_dict(copy.deepcopy(self.StructureToken.state_dict(), memo))

        # Copy the state of TaskToken
        new_instance.TaskToken = torch.nn.ModuleList([copy.deepcopy(task_token, memo) for task_token in self.TaskToken])

        # Other attributes can be shallow copied as they are not tensors
        new_instance.n_hidden = self.n_hidden
        new_instance.center_num = self.center_num
        new_instance.n_classes = self.n_classes
