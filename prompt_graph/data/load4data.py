...
import random
from torch_geometric.datasets import Planetoid, Amazon, Reddit, WikiCS, Flickr, Coauthor
...
def load4graph(dataset_name, shot_num= 10, num_parts=None, pretrained=False):
    ...
    if dataset_name in ['MUTAG', 'ENZYMES', 'COLLAB', 'PROTEINS', 'IMDB-BINARY', 'REDDIT-BINARY', 'COX2', 'BZR', 'PTC_MR']:
        ...
        if(pretrained==True):
            ...
            return input_dim, out_dim, train_data, val_dataset, test_dataset, dataset
...
def load4node(dataname, shot_num= 10):
    ...
    if dataname in ['Pubmed', 'Citeseer', 'Cora']:
        ...
        dataset = Coauthor(name = 'Physics', root='./data/',transform=NormalizeFeatures())
...
def NodePretrain(dataname='Citeseer', num_parts=100):
    if dataname in ['Pubmed', 'Citeseer', 'Cora']:
        dataset = Planetoid(root='data/Planetoid', name=dataname, transform=NormalizeFeatures())
    elif dataname in ['Computers', 'Photo']:
        dataset = Amazon(root='data/amazon', name=dataname)
    ...
    edge_index = data.edge_index
    edge_index = to_undirected(edge_index)
    data = Data(x=x, edge_index=edge_index, y = data.y)
    input_dim = data.x.shape[1]
    graph_cluster = ClusterData(data=data, num_parts=num_parts)
    graph_list = list(graph_cluster)

    return data, dataset, graph_list, input_dim, graph_cluster


def load4node_labelwise(args, dataname,train_ratio, test_ratio, device):
    if dataname in ['Pubmed', 'Citeseer', 'Cora']:
        dataset = Planetoid(root='data/Planetoid', name=dataname, transform=NormalizeFeatures())
    elif dataname in ['Computers', 'Photo']:
        dataset = Amazon(root='data/amazon', name=dataname)
    ...
    if(train_ratio+test_ratio >= 1):
        raise ValueError("the ratio of training set and test set should not larger or equal than 1.")
    print(f'Dataset: {dataset}:')
    print('======================')
    print(f'Number of graphs: {len(dataset)}')
    print(f'Number of features: {dataset.num_features}')
    ...
    split_train = get_split_self(num_samples=len(data), train_ratio=0.1, test_ratio=0.8,seed=args.seed, device=device)
