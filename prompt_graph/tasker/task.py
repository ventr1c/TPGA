...
class BaseTask:
    ...
    def initialize_optimizer(self):
        if self.prompt_type == 'None':
            ...
            self.pg_opi = optim.Adam(filter(lambda p: p.requires_grad, self.prompt.parameters()), lr=0.01, weight_decay= 0.00001)
            self.answer_opi = optim.Adam(filter(lambda p: p.requires_grad, self.answering.parameters()), lr=0.01, weight_decay= 0.00001)
